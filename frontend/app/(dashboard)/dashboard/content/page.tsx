  ...
  async function loadData() {
    try {
      ...
      if (contentCapability) {
        ...
        try {
          const scheduled = await apiClient.listScheduledPosts(digitalMarketer.id, true)
          setScheduledPosts(scheduled.scheduled_posts || [])
        } catch (error) {
    ...
    try {
      ...
      if (isScheduled) {
        ...
        if (scheduleType === "weekly") {
          scheduleConfig.days_of_week = [0, 1, 2, 3, 4, 5, 6] // All days by default
        } else if (scheduleType === "monthly") {
          scheduleConfig.days_of_month = [1] // First day by default
        }
      ...
      {scheduledPosts.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Automated content posting schedules</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
                ...
                >
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {sp.schedule_type} • Next: {new Date(sp.next_run_at).toLocaleString()}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Runs: {sp.successful_runs} successful, {sp.failed_runs} failed
                      </p>
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button 
              ...
              disabled={executing || !request.trim() || (isScheduled && (!scheduleName.trim() || !startDate))}
            >
              {executing ? (
                <>
